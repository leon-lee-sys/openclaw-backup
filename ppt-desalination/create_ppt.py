...
add_content_slide(prs, "1.3 研究目的", [
    ...
    ["揭示耦合机理与抗结垢机制", "构建多孔钛三维界面蒸发海水淡化系统", "深入揭示传质传热耦合机理"],
    ["探究孔隙结构对蒸发性能的影响", "揭示结垢规律并提出抗结垢策略", "为高效抗结垢海水淡化提供理论与工程指导"]
])
...
add_content_slide(prs, "2.1 研究内容概述", [
    ...
    ["材料制备与表征", "孔隙结构优化", "高比表面积设计"],
    ["耦合机理分析", "理论模型构建", "多物理场耦合研究"],
    ["结垢规律探究", "抗垢策略开发", "表面改性技术"]
])
...
add_content_slide(prs, "2.2 多孔钛三维界面技术", [
    ...
    ["高比表面积", "优异导热性", "良好耐腐蚀性", "可调控孔隙结构"],
    ["毛细作用增强", "蒸发面积最大化", "热传导优化", "蒸汽逃逸通道"],
    ["能耗降低50%以上", "常温（20-30°C）蒸发", "结垢显著减轻", "效率提高30%以上"]
])

# 第10页：技术对比
add_table_slide(prs, "2.3 技术性能对比", 
    ["指标", "传统技术", "本项目技术"],
    [
...
# 第12页：技术路线
add_flow_slide(prs, "3.1 总体技术路线", [
    "材料制备", "性能测试", "机理分析", "优化改进", "工程验证"
])
...
add_content_slide(prs, "3.2 材料制备与表征技术", [
    ...
    ["粉末冶金法", "溶胶-凝胶法", "3D打印法"],
    ["SEM形貌观察", "BET孔隙分析", "XRD晶相鉴定", "XPS表面化学分析"]
])
...
add_content_slide(prs, "3.4 计算模拟方法", [
    ...
    ["研究水分子与材料表面相互作用", "计算蒸发能垒", "分析离子分布规律"],
    ["温度场分布模拟", "速度场分析", "浓度场模拟"]
])
...
add_content_slide(prs, "3.5 理论模型构建", [
    ...
    ["基于能量守恒的能量方程", "基于质量守恒的质量方程", "考虑毛细作用的渗流模型"],
    ["结合孔结构和操作条件", "预测蒸发速率", "指导结构优化"]
])
...
# 第19页：创新点概览
add_content_slide(prs, "4.1 四大核心创新点", [
    "🌟 三维界面设计新概念 — 突破传统平面蒸发瓶颈",
    "🌟 多孔钛材料新应用 — 首创多孔钛三维骨架蒸发基底",
    "🌟 耦合机理新发现 — 首次系统揭示多物理场耦合机理",
    "🌟 抗结垢机制新策略 — 提出疏水-亲水图案化抗垢新方法"
])

# 第20页：创新点详解1
add_content_slide(prs, "4.2 创新点一：三维界面设计", [
    "核心创新：提出三维界面协同增效设计理念",
...
# 第21页：创新点详解2
add_content_slide(prs, "4.3 创新点二：多孔钛材料", [
    "核心创新：首创多孔钛三维骨架作为蒸发基底",
    "技术优势一：高导热性，加速热量传递",
    "技术优势二：高比表面积，增大蒸发面积",
    "技术优势三：优异耐腐蚀，适应海水环境"
])
...
# 第22页：创新点详解3
add_content_slide(prs, "4.4 创新点三：耦合机理", [
    "核心创新：首次系统揭示传热-传质-相变多物理场耦合机理",
    "建立定量预测模型",
    "发表高水平学术论文",
    "为后续研究奠定理论基础"
])
...
# 第23页：创新点详解4
add_content_slide(prs, "4.5 创新点四：抗结垢机制", [
    "核心创新：提出疏水-亲水图案化抗垢新策略",
    "结合外场辅助实现双重抗垢",
    "大幅延长运行周期",
    "显著降低维护成本"
])
...
# 第24页：关键技术指标
add_table_slide(prs, "4.6 关键技术指标目标", 
    ["指标", "当前水平", "目标值", "提升幅度"],
    [
        ["蒸发速率", "2-3 kg/m²·h", "≥5 kg/m²·h", "+70%"],
        ["能量效率", "50-60%", "≥85%", "+40%"],
        ["产水盐度", ">1000 mg/L", "<500 mg/L", "-50%"],
        ["运行周期", "7-10天", "≥30天", "+200%"],
        ["结垢抑制率", "20-30%", "≥70%", "+130%"]
    ])

# 第25页：预期成果
add_content_slide(prs, "4.7 预期研究成果", [
...
# 第26页：应用前景
add_content_slide(prs, "4.8 应用前景与市场", [
    "应用领域：海水淡化、苦咸水处理、废水回收利用",
    "市场规模：全球百亿美元级，中国千亿人民币级",
    "社会效益：缓解水资源短缺，助力碳中和目标",
    "产业链条：带动材料、装备、运维全产业链发展"
])
...
add_content_slide(prs, "5.3 阶段一：方案设计与材料制备（第1年）", [
    ...
    ["国内外研究现状调研", "技术路线确定", "实验方案设计"],
    ["多孔钛材料制备", "微观结构表征", "初步性能测试"]
])
...
add_content_slide(prs, "5.4 阶段二：实验研究（第2年）", [
    ...
    ["不同孔结构对比", "操作参数优化", "长期稳定性测试"],
    ["传热传质机理实验", "结垢机理研究", "抗垢策略验证"]
])
...
add_content_slide(prs, "5.5 阶段三/四：理论分析与成果总结（第3-4年）", [
    ...
    ["理论模型建立", "数值模拟验证", "高水平论文发表"],
    ["工程应用优化", "成本效益分析", "项目结题验收"]
])
...
output_path = os.path.join(output_dir, "三维界面冷蒸发海水淡化_PPT.pptx")
